...
def resolve_production_worker(harness_repo_url: str | None = None) -> WorkerAddress | None:
    ...
    ⚠️ 完整实现需查 evolution 的 harness_repo（按 production label 取版本）。
    本骨架返回 None（表示未配置路由，走本地直接装配）。

    实际接入：调 evolution API GET /api/harnesses?label=production → 版本 → resolve_worker_address。
    """
    # TODO（worker 部署后）：查 evolution 取 production 版本
