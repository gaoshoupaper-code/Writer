...
# 直接从源文件加载 linter 模块（它不在 executor/app 包里，无法常规 import）
...
def test_linter_scans_app_tree(linter):
    """scan() 应能扫描 executor/app/ 全树并返回结果（阶段 B 闭合后无 R5/R6 违规）。"""
    result = linter.scan()
    assert result.scanned > 0, "linter 至少应扫描到 executor/app/ 下的文件"
