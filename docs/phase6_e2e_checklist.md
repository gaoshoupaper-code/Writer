...
### 步骤 4：打开 manifest 开关（灰度切换）

在 `executor/.env` 加：
```ini
WRITER_USE_MANIFEST=true
```
...
**等价性对照**（可选但推荐）：
```bash
# 关 manifest 开关跑一次，开 manifest 开关跑一次，对比产出
# 关：WRITER_USE_MANIFEST=false（走旧 _assemble_via_harness）
# 开：WRITER_USE_MANIFEST=true（走 _assemble_via_manifest）
# 产出应等价（都是同样的 prompt/middleware/skills 装配）
```
...
- [ ] evolution 关闭时 executor 降级读缓存（不崩）
- [ ] `/internal/manifest/refreshed` 通知后 executor 标 stale

### 装配等价性
- [ ] `WRITER_USE_MANIFEST=true` 时生成不报错
- [ ] manifest 装配产出 = harness 装配产出（prompt/middleware/skills 一致）
...
### 降级
- [ ] manifest 拉取失败 → 降级到 harness 路径（日志有"降级"字样）
- [ ] 两档都关 → 走旧直接装配（不影响生产）
