"""surface 类型契约（共享包，零三方依赖）。

定义 surface 体系的「类型契约」：每个 surface_type 属于哪层（A/B/C）、
content 是什么形态（text/json/python）、归属哪个 scope。

本模块从 evolution/app/improvement/surface_registry.py 抽取，去掉了 validator 实现
（validator 是 evolution 私有逻辑，依赖 static_check，不进共享包）。
两端（executor/evolution）共享此处的类型定义，确保 surface 体系语义一致。
...
为什么是代码内置而非 DB：
  ...
  - 编译期可查：repo/loader 引用不存在的 surface_type 会直接 KeyError，
    而非运行时才发现。
...
class SurfaceTypeDef:
    """单个 surface_type 的类型定义（不含 validator，validator 由 evolution 注册）。"""
...
#
# 注意：此处不含 validator。validator（按 content_kind 分发到 text/json/python 校验）
# 由 evolution 端的 static_check 维护（VALIDATOR_MAP）。
...
# ── 查询 API（供 repo/proposer/static_check/executor loader 用）──────────
...
def get_type_def(surface_type: str) -> SurfaceTypeDef:
    ...
    if surface_type not in REGISTRY:
        raise KeyError(
            ...
            f" 加新类型需改 contracts.surface_types.REGISTRY（决策 D6：代码内置枚举）。"
        )
