...
create_deep_agent 实例，内部注册 review SubAgent 进行审查。
...
本模块只保留写作专属的装配逻辑（review + RevisionLimit + ArtifactValidation）。
"""
...
def build_deep_subagent(
    *,
    ...
    流程（由 DeepAgent 自主决策）：
      1. 接收父代理委托，生成/修改创作产物
      2. 调用 review 子代理审查产物质量
      3. 根据 review 返回的审查结果决定是否修订
      4. 若需修订，修订一次后不再二次审查（review 全程仅调用 1 次）
      5. 向父代理返回汇总结果
    ...
    Args:
        ...
        review_spec:         已构建好的 review SubAgent 规格字典
                             （由调用方通过 build_*_reviewer 构建）
