...
/**
 * LLM 配置面板（公共子组件，scope 分家 2026-07-18）。
 *
 * 接收 scope prop，所有 API 调用显式带 scope（T5=a）：
 *   - 'evolution'：进化 Agent（evaluate/evolve）评估侧
 *   - 'executor'：executor 给用户写正文侧
 * 同一套 CRUD 逻辑，数据按 scope 独立隔离。
 *
 * 不含 App 更新检查（D22：更新检查留在 EvolutionConfigPage 壳层）。
 */
...
export default function LlmConfigPanel({ scope }: LlmConfigPanelProps) {
  ...
  useEffect(() => {
    refresh();
    // scope 变化时重新拉（壳页面切换理论上不会复用实例，保险起见）
  }, [scope]);

  async function refresh() {
    try {
      const list = await listLlmConfigs(scope);
      setConfigs(list);
    } catch (err) {
  ...
  const scopeLabel = scope === "evolution" ? "进化端" : "执行端";
  ...
  return (
    <div className="config-page">
      ...
      <section className="config-status">
        {activeItem ? (
          <>
            <span className="status-badge ok">● {scopeLabel}已激活：{activeItem.name}</span>
            <span className="status-meta">
              {activeItem.model} · {activeItem.base_url}
            </span>
          </>
        ) : (
          <span className="status-badge warn">○ {scopeLabel}暂无激活配置</span>
        )}
      </section>
      ...
      <section className="config-list">
        {configs.length === 0 ? (
          <div className="config-empty">
            还没有{scopeLabel}配置。点击「+ 新建配置」添加你的第一个大模型 API。
          </div>
        ) : (
          configs.map((item) => (
