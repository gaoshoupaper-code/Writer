...
- promote_to_golden：growing→golden 升级（维护者独占，决策 D5/D17）
- get / list / get_golden_revision：查询
- update_demand_revision：golden 内容变更后更新锁定（A4）
"""
...
def promote_to_golden(case_id: str, *, demand_revision: str) -> dict[str, Any]:
    """升级 growing→golden（维护者独占，决策 D5/D17）。

    升级时记录 demand_revision（golden 锁定 git commit hash）。
    golden revision 是整个 golden 集的指纹——所有 golden case 共享同一值，
    故升级后统一刷新全部 golden case 的 demand_revision。
    返回升级后的元数据行。
    """
    now = _now()
    # 1. 升级目标 case
    db.execute(
        """UPDATE dataset_meta
           SET layer='golden', demand_revision=?, promoted_at=?, updated_at=?
           WHERE case_id=?""",
        (demand_revision, now, now, case_id),
    )
    # 2. 统一刷新所有 golden case 的 revision（保持一致）
    db.execute(
        """UPDATE dataset_meta SET demand_revision=?, updated_at=?
           WHERE layer='golden' AND status='active'""",
        (demand_revision, now),
    )
    return get(case_id)  # type: ignore[return-value]
