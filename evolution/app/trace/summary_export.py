"""模型可读的执行记录导出（从执行端移植，决策 D1）。

每条 trace 结束后自动生成一份摘要 JSON 文件，与原始 JSONL WAL 同目录。
摘要粒度为节点级别（agent/llm/tool/todo/error），每个节点只保留关键信息，
便于 LLM 高效阅读。通过 node_id / raw_event_ids 可回查原始 trace 事件。
"""
...
from app.core.models import TraceDetail, TraceNode
