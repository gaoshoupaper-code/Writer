...
class EvolveEventSink:
    ...
    def _handle_model_end(self, data: dict) -> list[str]:
        ...
        return [sse("model_output", evt_data)]

    def _handle_model_stream(self, data: dict) -> list[str]:
        ...
        return [sse("model_stream", {"content": content})]

    def _handle_tools_start(self, data: dict) -> list[str]:
    ...
    def _handle_tool_end(self, name: str, data: dict) -> list[str]:
        ...
        return [sse("tool_output", {
            "tool": name,
            "output_summary": output_str[:500],  # 截断防 SSE 拥堵（决策 R2）
            "call_id": call_id,
        })]

    def _handle_tool_error(self, name: str, data: dict) -> list[str]:
