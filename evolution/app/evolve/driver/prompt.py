...
def driver_system_prompt(
    ...
    Args:
        ...
        reflections_summary: 反思库摘要（历史失败模式，供 plan 子代理参考，D19）
    """
    reflections_section = ""
...
以下是历史评估中归纳的失败模式（按命中频率排序），plan 子代理设计改进方案时应参考：

{reflections_summary}

这些反思来自历史 badcase trace 的自动归纳。改进方案应针对这些高频失败模式。
"""
...
调用 task 委托给 plan 子代理：
  task(subagent_type="plan",
       prompt="读评估报告（read_eval_report）+ 读 trace（read_trace, trace_id={trace_id}），"
              ...
              {"参考历史失败反思，针对性改进高频失败模式。" if reflections_summary else ""})
