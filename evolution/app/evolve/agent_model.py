...
def build_evolve_model(*, temperature: float = 0.2) -> BaseChatModel:
    """构建进化 Agent 用的 ChatModel（复用 judge 配置，D7）。

    Args:
        temperature: 温度（进化决策需要一定探索性，默认 0.2）
    ...
    if not (settings.judge_model and settings.judge_api_key):
        raise RuntimeError(
            "进化 Agent 模型未配置：需设置 JUDGE_MODEL / JUDGE_API_KEY"
            "（复用 judge 配置，D7）"
        )
