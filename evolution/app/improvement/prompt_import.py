"""导入后端现有 prompt .md 到 evolution（Phase 4 Task 4.6）。

把后端分散的 prompt .md 文件导入为 evolution 的 version 1 + production label。
一次性导入，幂等（已存在的 prompt 跳过）。

用法：python -m app.improvement.prompt_import
"""
...
def import_backend_prompts(backend_root: Path) -> dict[str, int]:
    ...
    for name, rel_path in BACKEND_PROMPTS:
        ...
        if rel_path is None:
            # 内置常量导入（judge rubric）
            from app.diagnosis.rubric import DEFAULT_RUBRIC, OUTPUT_FORMAT
...
if __name__ == "__main__":
    # executor 根 = Writer/executor（本文件在 evolution/app/improvement/ 下，上溯四级到 Writer/）
    project_root = Path(__file__).resolve().parents[3]
    executor_root = project_root / "executor"
    if not executor_root.exists():
        print(f"执行端目录不存在: {executor_root}")
        raise SystemExit(1)
    print(f"从 {executor_root} 导入 prompt...")
    result = import_backend_prompts(executor_root)
