...
logger = logging.getLogger("evolution.harness_repo")

PRODUCTION_LABEL = "production"
LATEST_LABEL = "latest"
CANDIDATE_LABEL = "candidate"

# harness 代码存储根目录（默认 executor/harnesses，可被 settings 覆盖）
# 注意：evolution 和 executor 是两个服务，这里存的是「管理元数据指向的路径」，
# 实际代码文件由 executor 侧写入/读取。evolution 只记录路径 + 操作 git。
...
def write_harness_code(harnesses_root: Path, code: str, version_id: int) -> Path:
    ...
    ⚠️ 技术债（Phase 4 标注）：此处往 executor 目录写文件是文件系统耦合
    （evolution → executor）。当前不影响运行——executor 生产路径用写死的
    harnesses/v1，不读这些动态文件。worker 动态加载接通后，应改为 executor
    提供 harness 代码接收端点（HTTP），evolution 不再直接写 executor 目录。
    """
