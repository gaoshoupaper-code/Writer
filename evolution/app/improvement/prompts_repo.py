...
logger = logging.getLogger("evolution.prompts_repo")
...
def set_labels(version_id: int, labels: list[str]) -> None:
    ...
    Phase 5（D7 方案B）：若变更涉及 production label，通知执行端刷新缓存。
    """
    ...
    # production label 变更 → 通知执行端（D7 方案B）
    if PRODUCTION_LABEL in labels:
        _notify_executor_refresh(version_id, PRODUCTION_LABEL)


def _notify_executor_refresh(version_id: int, label: str) -> None:
    """通知执行端「某 prompt 版本上线了」，让它标记缓存 stale。

    纯副作用、彻底降级：executor_url 未配置 → 不通知；
    任何异常（网络/超时）→ 静默吞掉。执行端下次启动时会自然拉到新版，
    单次通知丢失只影响「及时性」不影响正确性。
    """
    try:
        ...
        if not executor_url:
            return
        # 反查 prompt name + version 号
        row = db.query_one(
            """SELECT p.name, pv.version
               FROM prompt_versions pv JOIN prompts p ON pv.prompt_id = p.id
               WHERE pv.id = ?""",
            (version_id,),
        )
        if row is None:
        ...
        import httpx
        httpx.post(
            f"{executor_url}/internal/prompts/refreshed",
            json={"name": row["name"], "version": row["version"], "label": label},
            timeout=2.0,
        )
    except Exception:
        logger.debug("通知执行端 prompt 刷新失败（version_id=%s）", version_id, exc_info=True)
