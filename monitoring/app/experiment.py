...
logger = logging.getLogger("monitoring.experiment")
...
def run_experiment(candidate_id: int, test_set_id: int | None = None) -> dict[str, Any] | None:
    ...
    try:
        ...
        for seed in range(SEED_COUNT):
            # production
            prod_trace = _run_one(backend_url, "production", req_item)
            if prod_trace:
                production_traces.append(prod_trace)
            # candidate
            cand_trace = _run_one(backend_url, "candidate", req_item)
            if cand_trace:
...
def _run_one(backend_url: str, label: str, req_item: dict[str, str]) -> str | None:
    """调后端 ab-replay 跑一次生成，返回 trace_id。"""
    try:
        resp = httpx.post(
            f"{backend_url}/internal/ab-replay",
            json={
                "prompt_label": label,
