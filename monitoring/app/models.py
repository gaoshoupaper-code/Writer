"""trace schema —— 从后端 platform/trace 复制（保持与运行时投影一致）。

monitoring 是事后离线投影，与后端运行时投影是两个关注点（见设计文档）。
schema 保持与后端完全一致，确保 events 能被 projector 正确投影。
"""
...
class TraceLogEvent(BaseModel):
    ...
    # 增量存储锚点（Phase 1 T1）：与后端 schemas 对齐，recorder 写入的稳定 anchor。
