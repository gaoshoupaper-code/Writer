...
实现策略（经真实 trace 验证修正）：
  ...
  - 内容读取：从 workspace 文件系统读（backend_workspace/{workspace_id}{path}）。
...
logger = logging.getLogger("monitoring.eval_extractor")
...
def _resolve_workspace_path(workspace_id: str, file_path: str) -> Path:
    ...
    workspace 根 = settings.backend_workspace_path（已解析为绝对路径，相对项目根）。
    子目录 = workspace_id。file_path 形如 /demand.md（前导 / 去除）。
    """
    rel = file_path.lstrip("/")
    return settings.backend_workspace_path / workspace_id / rel
