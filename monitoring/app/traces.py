...
from app.models import TraceDetail, TraceLogEvent, TraceNode, TraceRunSummary
...
def delete_trace(trace_id: str) -> dict[str, str]:
    """删除 trace 的 monitoring 记录（runs/nodes/events/flags 随级联删除）。"""
...
def _reconstruct_incremental_inputs(events: list[TraceLogEvent]) -> list[TraceLogEvent]:
    ...
    后端 recorder 把 LLM input 写成增量（Phase 1），monitoring 摄入保持增量存储
    （D4/D9 控空间）。详情视图/投影需要完整 input 时，顺着 anchor 链回溯重建。

    range 为空的事件（全量，T8）不受影响。无 llm_start 事件或链断裂时原样返回。
    """
    from app.increment import reconstruct_full_input
