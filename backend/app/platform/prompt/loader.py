...
职责：
  - 按 name + label 从 monitoring 拉 prompt（source of truth）
  - 本地缓存（monitoring 不可用时降级读缓存，T10 调和方案）
  - monitoring 未配置/不可用时降级读后端 .md 文件（向后兼容）
  - 拉取时把 version/label 暴露给调用方（T13：写进 trace）

设计依据：T10（monitoring主+后端缓存）、T13（版本进trace）。
"""
...
class PromptContent:
    ...
    version: int
    label: str
    source: str  # "monitoring" / "cache" / "local_file"（降级来源）


class PromptLoader:
    """统一 prompt 加载器。

    优先级：monitoring（远程）> 本地缓存 > 后端 .md 文件（降级）。
    降级是静默的：monitoring 不可用不影响 agent 运行。
    """
    ...
    def get(self, name: str, label: str = PRODUCTION) -> PromptContent:
        ...
        降级链：monitoring → 缓存 → 报错（无本地 .md 兜底，prompt 必须存在于 monitoring）。
        """
        # 1. 尝试 monitoring（远程）
        if self._monitoring_url:
            content = self._fetch_from_monitoring(name, label)
        ...
        if cached is not None:
            logger.warning("prompt %s 从缓存降级加载（monitoring 不可用）", name)
            return cached

        # 3. 都没有：报错（prompt 必须存在于 monitoring，运行前应导入）
        raise FileNotFoundError(
            f"Prompt '{name}' (label={label}) 未找到：monitoring 未配置或不可用，且无本地缓存。"
            f" 请运行 monitoring 的 python -m app.prompt_import 导入 prompt。"
        )

    def _fetch_from_monitoring(self, name: str, label: str) -> PromptContent | None:
        """从 monitoring HTTP 拉取。失败返回 None（触发降级）。"""
        try:
            import httpx

            url = f"{self._monitoring_url}/api/prompts/{name}?label={label}"
            resp = httpx.get(url, timeout=3.0)
            if resp.status_code == 404:
                logger.warning("prompt %s 在 monitoring 中不存在", name)
                return None
            ...
            logger.warning("从 monitoring 拉 prompt %s 失败，降级：%s", name, exc)
            return None
