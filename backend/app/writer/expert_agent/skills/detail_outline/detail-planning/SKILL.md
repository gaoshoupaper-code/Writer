...
description: >-
  章节细纲规划执行流程。读 timeline.md 取下一批事件，自主分章，
  写入 chapter-XX.md 并增量更新 overview.md，单次评估。
---

# detail-planning

章节细纲规划执行流程。读全局事件时间线，按固定事件批次自主分章，单次评估。
...
1. 调用 evolution 子代理评估本批章节质量（时间序正确性、爽点有效性、节奏疏密、人物一致性、与 timeline 一致性）。
2. evolution 读 detail/ 下文件 + storyline/timeline.md，写评估报告到 detail/evaluation.md，返回评分和修改建议。
3. 根据 evolution 返回结果：
   - "无需修改" → 直接返回。
